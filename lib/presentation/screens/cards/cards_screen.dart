...
const cardsDat = <Map<String,dynamic>> [
...
class _CardsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Column(
        children: [
          ...cardsDat.map((card) => _CardType1(elevation: 
              card['elevation'], 
              label: card['label'],
            ), 
          ),  
          ...cardsDat.map((card) => _CardType2(elevation: 
              card['elevation'], 
              label: card['label'],
            ), 
          ), 
          ...cardsDat.map((card) => _CardType3(elevation: 
              card['elevation'], 
              label: card['label'],
            ), 
          ), 
          ...cardsDat.map((card) => _CardType4(elevation: 
              card['elevation'], 
              label: card['label'],
            ), 
          ), 
...
class _CardType4 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card( 
      // clipBehavior => Evita que los hijos salgan de su padre
